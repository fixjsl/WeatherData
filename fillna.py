...
# 연도별 평균 기온
연평균 = df.groupby("년도")["평균기온(°C)"].mean()
연평균2 = df.groupby("년도")["최저기온(°C)"].mean()
연평균3 = df.groupby("년도")["최고기온(°C)"].mean()
연평균4 = df.groupby("년도")["합계 일조시간(hr)"].mean()
연평균5 = df.groupby("년도")["일강수량(mm)"].mean()
...
공통연도 = 연평균.index.intersection(개화일숫자.index)
상관관계 = 연평균.loc[공통연도].corr(개화일숫자.loc[공통연도])
...
공통연도 = 연평균4.index.intersection(개화일숫자.index)
상관관계4 = 연평균4.loc[공통연도].corr(개화일숫자.loc[공통연도])
공통연도 = 연평균5.index.intersection(개화일숫자.index)
상관관계5 = 연평균5.loc[공통연도].corr(개화일숫자.loc[공통연도])


print("개화일 vs 연평균기온 상관관계:", 상관관계)
print("개화일 vs 연평균기온 상관관계:", 상관관계2)

print("개화일 vs 연평균기온 상관관계:", 상관관계3)


print("개화일 vs 연평균기온 상관관계:", 상관관계4)

print("개화일 vs 연평균기온 상관관계:", 상관관계5)
